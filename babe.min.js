...
S<Q;S++){D(G+"/"+T[S],"datachange");}}}};Object.defineProperty(I,O,{enumerable:true,get:N,set:K});y[G][O]=1;}}function q(J){var L=this;if(v.indexOf(L.nodeName)<0){return;
...
}}else{b(M+">>>>%c控制器注册方法错误，没有遵循{uichange：fn,datachange:fn}格式","color:red");}}}}function x(G){["change","keyup"].forEach(function(H){G.addEventListener(H,q,false);
...
if(v.indexOf(R.nodeName)>=0){x(R);}for(var S in N){var L=N[S],O,P=o[S];if(P){j(K,Q,L);O=B(Q+"/"+L);if(typeof P.datachange=="function"){P.datachange.call(M,R,L,O);
}else{b(S+">>>>%c控制器注册方法错误，没有遵循{updatedata：fn,datachange:fn}格式","color:red");}}}});}}h.babe={bind:function(I,G){var H=g(I);H.setAttribute(k,I);C[I]=A(G);
G[i]=I;E[I]=G;p(I);},addController:function(H,G){o[H]=G;},getScope:n,getPathByDom:c,};}(this);